...
if (isset(filter_input_array(INPUT_POST)['cadastrar'])) {
    ...
    if ((empty($cpf)) || (!validaCPF($cpf)) || (leituraBD("cpf", "inscricao", "WHERE cpf='$cpf'")) 
            || (empty($senha)) || (empty($senhaConfirmar)) || ($senha != $senhaConfirmar) || (empty($nome)) 
            || (empty($dataNasc)) || (!validaData($dataNasc)) || (empty($telefone)) || (empty($email)) || (!filter_var($email, FILTER_VALIDATE_EMAIL))) {
    } else {
...
<html lang="pt-br">
    ...
    <body id="cadastrar">
        ...
        if (isset(filter_input_array(INPUT_POST)['cadastrar'])) {
            ...
            if ($inseriu) {
                echo funcAlert("PARABÉNS! Cadastro no SCE realizado com sucesso! Redirecionando para a página de Login."
                        . " Caso a página de Login não tenha carregado <a href='entrar.php'>clique aqui.</a>", "success");
            }
        }
        if ($cancelar) {
